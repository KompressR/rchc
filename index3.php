...
<?php					while ($event = mysqli_fetch_row($resultb)) { $eventstarttime = date_create_from_format('U', $event[6]); $thiseventstartdate = date_format($eventstarttime, 'Y-m-d'); $thiseventstarttime = date_format($eventstarttime, 'h:iA'); echo "				<p style=\"text-align:left; padding-top: 0px; margin-top: 0px;\"><span style=\"font-size: 1.45em\">".$thiseventstartdate." - ".$thiseventstarttime."</span><br><br>".$event[5]."<br><br>".$event[2]."<br>".$event[3]."<br><span style=\"font-weight: bold\">Price:</span> ".$event[4]."<br><span style=\"font-weight: bold\">Tickets:</span> <a href=\"".$event[8]."\">".$event[8]."</a><br><br></p>"; } ?>
				</div></div></div></div>
			</main>
	</body>
</html>
